...
function makeState(overrides: Partial<FamilyFeudInternalState> = {}): FamilyFeudInternalState {
  return {
    complexity: "standard",
    teamMode: false,
    teams: [],
...
describe("Fast Money Scoring", () => {
  it("awards bonus when total >= 200", () => {
    const totalPoints = 210;
    let bonusAwarded = false;

    if (totalPoints >= FAST_MONEY_BONUS_THRESHOLD) {
      bonusAwarded = true;
    }

    expect(bonusAwarded).toBe(true);
    expect(FAST_MONEY_BONUS_THRESHOLD).toBe(200);
    expect(FAST_MONEY_BONUS_POINTS).toBe(10000);
  });
  ...
  it("accumulates points from multiple fast money questions", () => {
    const answers = [
    ...
    expect(total < FAST_MONEY_BONUS_THRESHOLD).toBe(true);
  });
});
