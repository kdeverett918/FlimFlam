...
describe("Double Down values", () => {
  it("DOUBLE_DOWN_VALUES are exactly double the Round 1 values", () => {
    expect(DOUBLE_DOWN_VALUES).toEqual([400, 800, 1200, 1600, 2000]);
    for (let i = 0; i < CLUE_VALUES.length; i++) {
      expect(DOUBLE_DOWN_VALUES[i]).toBe((CLUE_VALUES[i] ?? 0) * 2);
    }
  });

  it("both value arrays have 5 entries (one per clue row)", () => {
    expect(CLUE_VALUES).toHaveLength(CLUES_PER_CATEGORY);
    expect(DOUBLE_DOWN_VALUES).toHaveLength(CLUES_PER_CATEGORY);
  });
  ...
  it("Round 2 values range from $400 to $2000", () => {
    expect(DOUBLE_DOWN_VALUES[0]).toBe(400);
    expect(DOUBLE_DOWN_VALUES[4]).toBe(2000);
  });
});

// ─── Power Play Placement Tests ─────────────────────────────────────────

describe("placePowerPlays", () => {
  it("places 1 power play for kids (Round 1)", () => {
    const count = getPowerPlayCount("kids");
    expect(count).toBe(1);
    const dd = placePowerPlays(count, CATEGORIES_PER_BOARD, CLUES_PER_CATEGORY);
    expect(dd.size).toBe(1);
  });

  it("places 1 power play for standard (legacy, Round 1 count)", () => {
    const count = getPowerPlayCount("standard");
    expect(count).toBe(1);
    const dd = placePowerPlays(count, CATEGORIES_PER_BOARD, CLUES_PER_CATEGORY);
    expect(dd.size).toBe(1);
  });

  it("places 1 power play for advanced (legacy, Round 1 count)", () => {
    const count = getPowerPlayCount("advanced");
    expect(count).toBe(1);
    const dd = placePowerPlays(count, CATEGORIES_PER_BOARD, CLUES_PER_CATEGORY);
    expect(dd.size).toBe(1);
  });

  it("can place 2 power plays (for Round 2)", () => {
    const dd = placePowerPlays(2, CATEGORIES_PER_BOARD, CLUES_PER_CATEGORY);
    expect(dd.size).toBe(2);
  });

  it("power plays are never on the $200 row (index 0)", () => {
  ...
  it("power plays are at valid positions", () => {
    for (let trial = 0; trial < 50; trial++) {
      const dd = placePowerPlays(2, CATEGORIES_PER_BOARD, CLUES_PER_CATEGORY);
      for (const posKey of dd) {
  ...
  it("power plays are unique positions", () => {
    for (let trial = 0; trial < 50; trial++) {
      const dd = placePowerPlays(2, CATEGORIES_PER_BOARD, CLUES_PER_CATEGORY);
      const positions = [...dd];
...
describe("validatePowerPlayWager", () => {
  it("accepts minimum wager of 5", () => {
    expect(validatePowerPlayWager(5, 1000)).toBe(true);
  });

  it("rejects wager below 5", () => {
    expect(validatePowerPlayWager(4, 1000)).toBe(false);
  });

  it("accepts wager equal to player score", () => {
    expect(validatePowerPlayWager(2000, 2000)).toBe(true);
  });

  it("rejects wager above player score when score > 1000", () => {
    expect(validatePowerPlayWager(3000, 2000)).toBe(false);
  });
  ...
  it("allows wager up to 1000 when player score is low", () => {
    ...
    expect(validatePowerPlayWager(1000, 200)).toBe(true);
  });

  it("allows wager up to 1000 when player score is 0", () => {
    expect(validatePowerPlayWager(1000, 0)).toBe(true);
  });

  it("allows wager up to 1000 when player score is negative", () => {
    expect(validatePowerPlayWager(1000, -500)).toBe(true);
  });

  it("rejects non-finite wagers", () => {
    expect(validatePowerPlayWager(Number.NaN, 1000)).toBe(false);
    expect(validatePowerPlayWager(Number.POSITIVE_INFINITY, 1000)).toBe(false);
  });

  it("allows wager up to 2000 in Double Down when player score is low", () => {
    // With highestClueValue=2000, max wager = max(score, 2000)
    expect(validatePowerPlayWager(2000, 200, 2000)).toBe(true);
  });

  it("rejects wager above 2000 in Double Down when score is low", () => {
    expect(validatePowerPlayWager(2001, 200, 2000)).toBe(false);
  });
});

describe("validateAllInWager", () => {
  it("accepts wager of 0", () => {
    expect(validateAllInWager(0, 5000)).toBe(true);
  });

  it("accepts wager equal to score", () => {
    expect(validateAllInWager(5000, 5000)).toBe(true);
  });

  it("rejects wager above score", () => {
    expect(validateAllInWager(6000, 5000)).toBe(false);
  });

  it("rejects negative wager", () => {
    expect(validateAllInWager(-1, 5000)).toBe(false);
  });

  it("rejects any wager when player score is 0", () => {
    expect(validateAllInWager(0, 0)).toBe(false);
  });

  it("rejects any wager when player score is negative", () => {
    expect(validateAllInWager(0, -100)).toBe(false);
  });

  it("rejects non-finite wagers", () => {
    expect(validateAllInWager(Number.NaN, 5000)).toBe(false);
    expect(validateAllInWager(Number.POSITIVE_INFINITY, 5000)).toBe(false);
  });
});
...
describe("scoring mechanics", () => {
  ...
  it("power play correct awards wager amount (conceptual)", () => {
    const score = 2000;
    const wager = 1500;
  ...
  it("power play wrong deducts wager amount (conceptual)", () => {
    const score = 2000;
    const wager = 1500;
    const newScore = score - wager;
  ...
  it("all-in round correct adds wager (conceptual)", () => {
    const score = 5000;
    const wager = 3000;
    const newScore = score + wager;
  ...
  it("all-in round wrong deducts wager (conceptual)", () => {
    const score = 5000;
    const wager = 3000;
    const newScore = score - wager;
  ...
  it("Double Down values are used in Round 2 scoring (conceptual)", () => {
    // In Round 2, a row-0 clue is worth $400 instead of $200
    const round2Value = DOUBLE_DOWN_VALUES[0];
...
// ─── All-In Round Eligibility Tests ─────────────────────────────────────

describe("all-in round eligibility", () => {
...
describe("round-robin selector rotation", () => {
  ...
  it("power play does not break rotation — still advances after (conceptual)", () => {
    const playerOrder = ["p1", "p2", "p3"];
    let selectorIndex = 1; // p2 is selector, hits a Power Play
...
describe("createBrainBoardPlugin", () => {
  ...
  it("creates a plugin with the correct manifest", async () => {
    ...
    const plugin = createBrainBoardPlugin();

    expect(plugin.manifest.id).toBe("brain-board");
    expect(plugin.manifest.name).toBe("Brain Board");
    expect(plugin.manifest.minPlayers).toBe(2);
