...
describe("spinWheel", () => {
  ...
  it("contains 2 bankrupt segments", () => {
    const bankrupts = WHEEL_SEGMENTS.filter((s) => s.type === "bankrupt");
    expect(bankrupts).toHaveLength(2);
  });

  it("contains 1 lose-a-turn segment", () => {
    const loseATurn = WHEEL_SEGMENTS.filter((s) => s.type === "lose-a-turn");
    expect(loseATurn).toHaveLength(1);
  });

  it("contains 1 free-play segment", () => {
    const freePlay = WHEEL_SEGMENTS.filter((s) => s.type === "free-play");
    expect(freePlay).toHaveLength(1);
  });
});
...
// ─── Bankrupt Resets Round Cash ────────────────────────────────────────

describe("bankrupt mechanic", () => {
  it("bankrupt segment has value 0", () => {
    const bankruptSegments = WHEEL_SEGMENTS.filter((s) => s.type === "bankrupt");
    for (const s of bankruptSegments) {
      expect(s.value).toBe(0);
    }
  });

  it("bankrupt loses round cash (conceptual test -- cash tracking is in the plugin)", () => {
    // Simulate the cash tracking logic
    const _roundCash = 3000;
    const totalCash = 5000;
    // On bankrupt: roundCash -> 0, totalCash stays the same
    const afterBankrupt = { roundCash: 0, totalCash };
    expect(afterBankrupt.roundCash).toBe(0);
    expect(afterBankrupt.totalCash).toBe(5000);
  });
});
...
describe("turn advancement", () => {
  ...
  it("wrong consonant guess loses turn (conceptual)", () => {
    // If a player guesses a consonant not in the puzzle and it's not Free Play,
...
describe("free play mechanic", () => {
  it("free play segment has a cash value", () => {
    const freePlaySegments = WHEEL_SEGMENTS.filter((s) => s.type === "free-play");
    expect(freePlaySegments).toHaveLength(1);
    expect(freePlaySegments[0]?.value).toBeGreaterThan(0);
  });

  it("wrong guess during free play does not lose turn (conceptual)", () => {
    // Free play means wrong consonant guess doesn't advance the turn.
    // The plugin handles this via the freePlayActive flag.
    // We just verify the segment type is correctly identified.
    const freePlaySegment = WHEEL_SEGMENTS.find((s) => s.type === "free-play");
    expect(freePlaySegment).toBeDefined();
    expect(freePlaySegment?.type).toBe("free-play");
  });
});
