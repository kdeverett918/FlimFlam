...
class WheelOfFortunePlugin extends BaseGamePlugin {
  ...
  private handleGuessConsonant(
    ...
    if (count > 0) {
      ...
      if (this.freePlayActive) {
        // Free play: wrong guess doesn't lose turn
        this.scheduleDelayed(room, LETTER_RESULT_DELAY_MS, () => {
          this.goToPlayerChoice(room, state);
        });
      } else {
  ...
  private broadcastGameState(room: Room, _state: Schema): void {
    ...
    room.broadcast("game-data", {
      ...
      freePlayActive: this.freePlayActive,
...
/** Factory function to create a Wheel of Fortune game plugin. */
export function createWheelOfFortunePlugin(): WheelOfFortunePlugin {
  return new WheelOfFortunePlugin();
}
