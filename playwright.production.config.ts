...
const e2eHostPort = process.env.FLIMFLAM_E2E_HOST_PORT ?? "3302";
const e2eControllerPort = process.env.FLIMFLAM_E2E_CONTROLLER_PORT ?? "3303";
...
process.env.FLIMFLAM_E2E_COLYSEUS_HEALTH_URL = `${PRODUCTION_COLYSEUS_HTTP}/health`;
...
export default defineConfig({
  ...
  webServer: {
    ...
    // host + controller (FLIMFLAM_E2E_SKIP_SERVER=1 skips the local Colyseus process).
    ...
    env: {
      ...
      FLIMFLAM_E2E_SKIP_SERVER: "1",
      // Point the in-test health poll at the production server.
      FLIMFLAM_E2E_COLYSEUS_HEALTH_URL: `${PRODUCTION_COLYSEUS_HTTP}/health`,
      FLIMFLAM_TIMER_SCALE: process.env.FLIMFLAM_TIMER_SCALE ?? "0.2",
